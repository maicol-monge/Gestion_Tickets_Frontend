...
const MisAsignaciones = () => {
  ...
  useEffect(() => {
    const cargarDatosIniciales = async () => {
      ...
      try {
        const catResponse = await fetch("https://localhost:7106/api/Filtro/obtener-categorias");
  ...
  try {
    ...
    const response = await fetch(`https://localhost:7106/api/Filtro/mis-asignaciones?${params.toString()}`);
  ...
  <div fluid className="my-4 px-4">
      ...
      <div className="mt-4">
        {loading ? (
          ...
          <Alert variant="danger">Error al cargar las asignaciones: {error}</Alert>
        ) : tickets.length === 0 ? (
          <Alert variant="info">No se encontraron asignaciones para tu usuario</Alert>
        ) : (
